...
router.post("/posts", async (req, res) => {
  // const index = await Posts.find().count() + 1;
  const { postId, title, name, password, content } = req.body;

  const posts = await Posts.find({ index });
  if (posts.length) {
  ...
  // const createdPosts = await Posts.create({ postId, title, name, password, content, createdAt });
